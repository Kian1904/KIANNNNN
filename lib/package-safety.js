...
function extractPackageName(command) {
  // handle: npm install X / npm i X / npm i -D X / npm install X@version
  const npmMatch = command.match(/npm\s+(?:install|i)\s+(?:-[A-Za-z]+\s+)*([a-zA-Z0-9@/_.-]+)/);
  if (npmMatch) {
    // buang versi kalau ada (chalk@4.1.2 -> chalk), tapi biarkan scoped package (@scope/name)
...
async function checkNpm(name) {
  ...
  try {
    ...
    if (firstPublish) {
      const ageDays = (Date.now() - new Date(firstPublish).getTime()) / 86400000;
      if (ageDays < 30) {
  ...
  try {
    ...
    if (dlRes.ok) {
      ...
      const downloads = dlData.downloads || 0;
        dbg(`downloads/week: ${downloads.toLocaleString()}, age: ${Math.floor(ageDays)} days`);
...
export async function checkPackageSafety(command) {
  ...
  if (!extracted) return null; // bukan install command
