...
export const askHuggingFace = makeOpenAICompatible({
  name: 'HuggingFace',
...
const PROVIDER_ORDER = [
  { name: 'Deepseek-R1', fn: askHuggingFace },
  { name: 'Qwen3-coder-plus', fn: askXKiroCoder },
  { name: 'Gemini 3.1 flash-lite', fn: askGemini },
  { name: 'Deepseek-V4-Pro', fn: askXKiro },
  { name: 'Nemotron 3 ultra', fn: askOpenRouter },
  { name: 'Laguna xs2.1', fn: askNvidia },
  { name: 'Mistral small-latest', fn: askMistral },
  { name: 'Qwen3.6-27b', fn: askGroq}
];
...
export const PROVIDER_NAMES = [
  { key: 'huggingface', label: 'HuggingFace(DeepSeek-R1)' },
  { key: 'xkiro-coder', label: 'xKiro-Coder (Qwen3-Coder-plus)' },
  { key: 'xkiro',        label: 'xKiro(DeepSeek-R1)' },
  { key: 'openrouter',  label: 'OpenRouter (Nemotron-550B)' },
  { key: 'gemini',      label: 'Gemini (3 1 Flash-lite)' },
...
export async function askWithFallback(systemPrompt, userPrompt) {
 ...
 const providerMap = {
  'xkiro': askXKiro, 'xkiro-coder': askXKiroCoder,
  'huggingface': askHuggingFace, 'openrouter': askOpenRouter,
  'gemini': askGemini, 'mistral': askMistral
};
const defaultOrder = [askXKiro, askXKiroCoder, askHuggingFace, askOpenRouter, askGemini, askMistral];
const primary = _primaryKey && providerMap[_primaryKey];
const providers = primary
  ? [primary, ...defaultOrder.filter(p => p !== primary)]
  : defaultOrder;
