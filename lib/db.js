...
export function logStep({ threadId, role, content, actionType, detail, reasoning, approved }) {
  ...
  // Deteksi kolom "warisan" schema lama yang mungkin masih NOT NULL di tabel fisik lo
  // (session_id gak ada di CREATE TABLE versi sekarang, tapi kalau db lo pernah dibuat
  // dari versi lebih lama yang punya kolom itu, INSERT wajib ngisi biar gak kena
  // NOT NULL constraint).
  ...
  if (hasSessionId) {
    return db.prepare(`
      INSERT INTO conversations (thread_id, role, content, action_type, detail, reasoning, approved, session_id)
      VALUES (@threadId, @role, @content, @actionType, @detail, @reasoning, @approved, @sessionId)
    `).run({
      threadId,
      role,
      content,
